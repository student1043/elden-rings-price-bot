...
async def get_price(ctx, *args):
    list_pricing = []
    channel = bot.get_channel(1024849506258407454)
    url = "https://eu.tamrieltradecentre.com/pc/Trade/PriceCheckResult?ItemNamePattern="
    if len(args) > 1:
        search_value = "+".join(args) 
    driver = webdriver.Firefox()
    driver.get(search_value)
    soup = BeautifulSoup(page.text, 'html.parser')
    for item in soup.find_all(class_="gold-amount"):
        print(item)
    await ctx.channel.send(list_pricing)
